...
tyrano.plugin.kag.tag.bgmopt = {
    pm: {volume: "100", effect: "true", buf: ""}, start: function (pm) {
        ...
        if ("" != pm.buf) this.kag.stat.map_bgm_volume[pm.buf] = pm.volume; else {
        ...
        if ("true" == pm.effect && 0 == this.kag.define.FLAG_APRI) {
            ...
            if ("" == pm.buf) for (key in map_bgm) map_bgm[key] && map_bgm[key].volume(new_volume); else map_bgm[pm.buf] && map_bgm[pm.buf].volume(new_volume)
        }
