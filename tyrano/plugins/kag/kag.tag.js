tyrano.plugin.kag.ftag = {
    tyrano: null, kag: null, array_tag: [], master_tag: {}, current_order_index: -1, init: function () {
        ...
        if ("false" == this.kag.stat.flag_glyph) {
            $(".img_next").remove();
            this.kag.getMessageInnerLayer().find("p").append("<img class='img_next' src='./tyrano/images/system/" + this.kag.stat.path_glyph + "' />")
        ...
        if ("call" == tag.name && "make.ks" == tag.pm.storage || "make.ks" == this.kag.stat.current_scenario || "call" == tag.name && tag.pm.storage == this.kag.stat.resizecall.storage || this.kag.stat.current_scenario == this.kag.stat.resizecall.storage) {
        ...
        if (1 == this.checkCond(tag)) {
            ...
            if (this.master_tag[tag.name]) {
                ...
                if ("" != err_str) this.kag.error(err_str); else {
        ...
        return !!this.master_tag[tag.name].cw && (1 != this.kag.stat.is_script && 1 != this.kag.stat.is_html && 1 != this.kag.stat.checking_macro)
        ...
        for (key in pm) {
            ...
            if (val.length > 0 && "&" === c) pm[key] = this.kag.embScript(val.substr(1, val.length)); else if (val.length > 0 && "%" === c) {
        ...
        for (var array_vital = master_tag.vital, i = 0; i < array_vital.length; i++) tag.pm[array_vital[i]] ? "" == tag.pm[array_vital[i]] && (err_str += "Метка「" + tag.name + "」на параметр「" + array_vital[i] + "」является обязательной　\n") : err_str += "Метка「" + tag.name + "」на параметр「" + array_vital[i] + "」является обязательной　\n";
        return err_str
        ...
        if ((scenario_file = scenario_file || this.kag.stat.current_scenario) != this.kag.stat.current_scenario || 1 == flag) {
            ...
            if ("yes" == auto_next) this.nextOrder(); else if ("snap" == auto_next) {
    ...
    cw: !0, pm: {val: "", backlog: "add"}, start: function (pm) {
            ...
            if ("true" == this.kag.stat.vertical) {
                if ("false" != this.kag.config.defaultAutoReturn) {
                    var j_outer_message = this.kag.getMessageOuterLayer(),
                        limit_width = .8 * parseInt(j_outer_message.css("width"));
        ...
        !function (jtext) {
            ...
            for (var append_str = "", i = 0; i < message_str.length; i++) {
                ...
                if (" " == c) append_str += "<span style='opacity:0'>" + c + "</span>"; else {
                    if (1 == that.kag.stat.mark) {
                        c = "<mark style='" + that.kag.stat.style_mark + "'>" + c + "</mark>"
            ...
            if (that.kag.stat.fuki.active) {
                ...
                if ("" == chara_name) is_chara_show = !1; else {
                    ...
                    if (void 0 !== chara_obj && chara_obj.get(0) && "true" == that.kag.stat.charas[original_name].fuki.enable) {
                        is_chara_show = !0;
                        ...
                        let width = j_msg_inner.css("width"), height = j_msg_inner.css("height");
                        ...
                        (j_outer_message = that.kag.getMessageOuterLayer()).css("width", width);
                        j_outer_message.css("height", height);
                        chara_left = parseInt(chara_obj.css("left"));
                        chara_top = parseInt(chara_obj.css("top"));
                        let fuki_left = chara_fuki.left, fuki_top = chara_fuki.top,
                            chara_width = (chara_fuki.sippo_left, chara_fuki.sippo_top, parseInt(chara_obj.find("img").css("width"))),
                        ...
                        let outer_width = parseInt(j_outer_message.css("width")),
                            outer_height = parseInt(j_outer_message.css("height")), sippo = chara_fuki.sippo;
                        "bottom" == sippo ? fuki_top2 -= outer_height : "left" == sippo ? fuki_left2 += parseInt(chara_fuki.sippo_left) : "right" == sippo && (fuki_left2 -= outer_width);
                        let fuki_right = fuki_left2 + outer_width, fuki_bottom = fuki_top2 + outer_height,
                            sc_width = parseInt(that.kag.config.scWidth),
                            sc_height = parseInt(that.kag.config.scHeight), sippo_left = 0;
                        if (fuki_right >= sc_width) {
                            fuki_left2 = fuki_left2 - (fuki_right - sc_width) - 10;
                            sippo_left = fuki_right - sc_width + 10
                        }
                        fuki_bottom >= sc_height && (fuki_top2 = fuki_top2 - (fuki_bottom - sc_height) - 10);
                        if (fuki_left2 <= 0) {
                ...
                if (0 == is_chara_show) {
                    $(".tyrano_base").find(".chara_name_area").hide();
                    let others_style = that.kag.stat.fuki.others_style, def_style = that.kag.stat.fuki.def_style,
                        nwidth = others_style.max_width || def_style.width, nleft = others_style.left || def_style.left,
            ...
            var append_span = j_span.children("span:last-child"), pchar = function (index) {
                ...
                if (index <= message_str.length) {
                    ...
                    1 == that.kag.stat.is_click_text || 1 == that.kag.stat.is_skip || 1 == that.kag.stat.is_nowait ? pchar(++index) : setTimeout((function () {
                        pchar(++index)
                    }), ch_speed)
                } else {
                    that.kag.stat.is_adding_text = !1;
                    that.kag.stat.is_click_text = !1;
                    if ("true" != that.kag.stat.is_stop) if (isOneByOne) that.kag.stat.is_hide_message || that.kag.ftag.nextOrder(); else {
        ...
        void 0 !== chara_fuki.border_color && j_outer_message.css("border-color", $.convertColor(chara_fuki.border_color));
        void 0 !== chara_fuki.radius && j_outer_message.css("border-radius", parseInt(chara_fuki.radius));
        void 0 !== chara_fuki.font_color && j_outer_message.parent().find(".message_inner").find(".current_span").css("color", $.convertColor(chara_fuki.font_color));
        void 0 !== chara_fuki.font_size && j_outer_message.parent().find(".message_inner").find(".current_span").css("font-size", parseInt(chara_fuki.font_size))
    }
};
    ...
    pm: {nextorder: "true"}, start: function (pm) {
        if ("true" == this.kag.config.autoRecordLabel) {
            var sf_tmp = "trail_" + this.kag.stat.current_scenario.replace(".ks", "").replace(/\u002f/g, "").replace(/:/g, "").replace(/\./g, ""),
                sf_buff = this.kag.stat.buff_label_name, sf_label = sf_tmp + "_" + pm.label_name;
...
tyrano.plugin.kag.tag.l = {
    cw: !0, start: function () {
        ...
        if (1 == this.kag.stat.is_skip) this.kag.ftag.nextOrder(); else if (1 == this.kag.stat.is_auto) {
            ...
            setTimeout((function () {
                1 == that.kag.stat.is_wait_auto && (1 == that.kag.tmp.is_vo_play ? that.kag.tmp.is_vo_play_wait = !0 : that.kag.ftag.nextOrder())
            }), auto_speed)
        }
    }
};
...
tyrano.plugin.kag.tag.r = {
    log_join: "true", start: function () {
        var that = this, j_inner_message = this.kag.getMessageInnerLayer(),
...
tyrano.plugin.kag.tag.position = {
    pm: {
        ...
        "" != pm.radius && (new_style["border-radius"] = parseInt(pm.radius) + "px");
        if ("" != pm.border_size) {
            new_style["border-width"] = parseInt(pm.border_size) + "px";
            target_layer.css("border-style", "solid")
        }
        "" != pm.border_color && (new_style["border-color"] = $.convertColor(pm.border_color));
        if ("none" == pm.frame) {
        ...
        this.kag.stat.fuki.def_style = $.extend(!0, this.kag.stat.fuki.def_style, new_style);
        ...
        this.kag.stat.fuki.def_style_inner = $.extend(!0, this.kag.stat.fuki.def_style_inner, new_style_inner);
        "true" == pm.next && this.kag.ftag.nextOrder()
    }
};
tyrano.plugin.kag.tag.fuki_start = {
    pm: {layer: "message0", page: "fore"}, start: function (pm) {
...
tyrano.plugin.kag.tag.fuki_chara = {
    ...
    start: function (pm) {
        ...
        if ("others" == pm.name) this.kag.stat.fuki.others_style = $.extend(this.kag.stat.fuki.others_style, pm); else {
            var cpm = this.kag.stat.charas[pm.name];
            if (null == cpm) {
                this.kag.error("Указанный персонаж「" + pm.name + "」не определен через [chara_new]");
                return
            }
            let _cpm = cpm.fuki;
    ...
    pm: {
        ...
        if ("base" != pm.layer) {
            ...
            if ("" != pm.time) {
                ...
                img_obj.stop(!0, !0).animate({opacity: 1}, parseInt(pm.time), (function () {
            ...
            var new_style = {
                "background-image": "url(" + (strage_url = $.isHTTP(pm.storage) ? pm.storage : "./data/" + folder + "/" + pm.storage) + ")",
                display: "none"
            };
    ...
    start: function (pm) {
        ...
        if ("base" != pm.layer) {
            ...
            if ("" != pm.time) {
                ...
                j_obj.stop(!0, !0).animate({opacity: 0}, parseInt(pm.time), (function () {
...
tyrano.plugin.kag.tag.quake = {
    vital: ["time"],
    pm: {count: 5, time: 300, timemode: "", hmax: "0", vmax: "10", wait: "true"},
    start: function (pm) {
        var that = this;
        "0" != pm.hmax ? $("." + this.kag.define.BASE_DIV_NAME).effect("shake", {
            times: parseInt(pm.count),
            distance: parseInt(pm.hmax),
            direction: "left"
        }, parseInt(pm.time), (function () {
            "true" == pm.wait && that.kag.ftag.nextOrder()
        })) : "0" != pm.vmax && $("." + this.kag.define.BASE_DIV_NAME).effect("shake", {
            times: parseInt(pm.count),
...
tyrano.plugin.kag.tag.layopt = {
    vital: ["layer"],
    pm: {layer: "", page: "fore", visible: "", left: "", top: "", opacity: "", autohide: !1, index: 10},
    start: function (pm) {
        if ("message" == pm.layer) {
...
tyrano.plugin.kag.tag.mark = {
    vital: [], pm: {color: "0xFFFF00", font_color: "", size: ""}, start: function (pm) {
        pm.text;
        this.kag.stat.mark = 1;
        var style_mark = "margin-right:-1px;";
        style_mark += "background-color:" + $.convertColor(pm.color) + ";";
        "" != pm.font_color ? style_mark += "color:" + $.convertColor(pm.font_color) + ";" : style_mark += "color:" + this.kag.stat.font.color + ";";
        "" != pm.size && (style_mark += "background: linear-gradient(transparent " + (100 - parseInt(pm.size)) + "%, " + $.convertColor(pm.color) + " 0%);");
...
tyrano.plugin.kag.tag.button = {
    pm: {
        ...
        var that = TYRANO;
        !function () {
            var _target = pm.target, _storage = pm.storage, _pm = pm, preexp = that.kag.embScript(pm.preexp),
                button_clicked = !1, parse_img_url = function (src) {
            ...
            j_button.click((function (event) {
                ...
                if ("" == _pm.role && "true" == _pm.fix) {
                    ...
                    if (null != stack_pm) {
                        that.kag.log("Если стек вызовов остается, кнопка исправления не срабатывает");
...
tyrano.plugin.kag.tag.glink = {
    pm: {
        ...
        if ("auto" == pm.x) {
            var sc_width = parseInt(this.kag.config.scWidth), center = Math.floor(parseInt(j_button.css("width")) / 2),
...
tyrano.plugin.kag.tag.glyph = {
    pm: {line: "nextpage.gif", layer: "message0", fix: "false", left: 0, top: 0},
    start: function (pm) {
        $(".glyph_image").remove();
        if ("true" == pm.fix) {
...
tyrano.plugin.kag.tag.bg = {
    vital: ["storage"],
    pm: {storage: "", method: "crossfade", wait: "true", time: 3e3, cross: "false", position: ""},
    start: function (pm) {
        this.kag.ftag.hideNextImg();
        var that = this;
        ...
        this.kag.preload(storage_url, (function () {
            ...
            "true" == pm.cross && $.trans(pm.method, j_old_bg, parseInt(pm.time), "hide", (function () {
                j_old_bg.remove()
            }));
...
tyrano.plugin.kag.tag.bg2 = {
    ...
    start: function (pm) {
        ...
        this.kag.preload(storage_url, (function () {
            ...
            var scWidth = parseInt(that.kag.config.scWidth), scHeight = parseInt(that.kag.config.scHeight), left = 0,
                top = 0;
...
tyrano.plugin.kag.tag.layermode = {
    vital: [],
    pm: {name: "", graphic: "", color: "", mode: "multiply", folder: "", opacity: "", time: "500", wait: "true"},
    start: function (pm) {
        this.kag.ftag.hideNextImg();
        ...
        "" != pm.graphic ? this.kag.preload(storage_url, (function () {
...
tyrano.plugin.kag.tag.layermode_movie = {
    ...
    start: function (pm) {
        this.kag.ftag.hideNextImg();
        var that = this, blend_layer = null,
            video = (blend_layer = $("<video class='layer_blend_mode blendlayer blendvideo' data-video-name='" + pm.name + "' data-video-pm='' style='display:none;position:absolute;width:100%;height:100%;z-index:99' ></video>")).get(0),
        ...
        $("#tyrano_base").append(blend_layer);
        blend_layer.stop(!0, !0).fadeIn(parseInt(pm.time), (function () {
            "true" == pm.wait && "true" == pm.loop && "true" != pm.stop && that.kag.ftag.nextOrder()
        }));
...
tyrano.plugin.kag.tag.free_layermode = {
    vital: [], pm: {name: "", time: "500", wait: "true"}, start: function (pm) {
        this.kag.ftag.hideNextImg();
        var that = this, blend_layer = {},
            cnt = (blend_layer = "" != pm.name ? $(".layer_blend_" + pm.name) : $(".blendlayer")).length, n = 0;
        if (0 != cnt) {
