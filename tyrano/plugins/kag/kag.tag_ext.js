tyrano.plugin.kag.tag.loadjs = {
    vital: ["storage"], pm: {storage: ""}, start: function (pm) {
        var that = this;
...
tyrano.plugin.kag.tag.movie = {
    ...
    playVideo: function (pm) {
        var that = this, url = "./data/video/" + pm.storage, video = document.createElement("video");
        video.id = "bgmovie";
        video.src = url;
        ...
        console.log(pm.control_buttons)
        if (pm.control_buttons == "true") {
            video.setAttribute("controls", "");
        }
        if (pm.transparentBackground == "false") {
        ...
        if ("true" == pm.bgmode) {
            ...
            video.addEventListener("ended", (function (e) {
                if (null == that.kag.stat.video_stack) {
                    ...
                    var video_pm = that.kag.stat.video_stack, video2 = document.createElement("video");
...
tyrano.plugin.kag.tag.bgmovie = {
    vital: ["storage"],
    pm: {storage: "", volume: "", loop: "true", mute: "false", time: "300", stop: "false", transparentBackground: ""},
    start: function (pm) {
        pm.skip = "false";
        pm.bgmode = "true";
...
tyrano.plugin.kag.tag.wait_bgmovie = {
    vital: [], pm: {stop: "false"}, start: function (pm) {
...
tyrano.plugin.kag.tag.stop_bgmovie = {
    vital: [], pm: {time: "300", wait: "true"}, start: function (pm) {
        var that = this;
        that.kag.tmp.video_playing = !1;
        that.kag.stat.current_bgmovie.storage = "";
        that.kag.stat.current_bgmovie.volume = "";
        $(".tyrano_base").find("video").animate({opacity: "0"}, {
            duration: parseInt(pm.time), complete: function () {
                $(this).remove();
...
tyrano.plugin.kag.tag.autostop = {
    pm: {next: "true"}, start: function (pm) {
...
tyrano.plugin.kag.tag.autoconfig = {
    pm: {speed: "", clickstop: ""}, start: function (pm) {
        if ("" != pm.speed) {
            this.kag.config.autoSpeed = pm.speed;
            this.kag.ftag.startTag("eval", {exp: "sf._system_config_auto_speed = " + pm.speed, next: "false"})
        }
        if ("" != pm.clickstop) {
            this.kag.config.autoClickStop = pm.clickstop;
            this.kag.ftag.startTag("eval", {exp: "sf._system_config_auto_click_stop = " + pm.clickstop, next: "false"})
        }
        this.kag.ftag.nextOrder()
    }
};
...
tyrano.plugin.kag.tag.stopanim = {
    vital: ["name"], pm: {name: ""}, start: function (pm) {
        $("." + pm.name).stop();
...
tyrano.plugin.kag.tag.frame = {
    ...
    start: function (pm) {
        var percentage = pm.p;
        delete pm.p;
        if (this.kag.stat.map_keyframe[this.kag.stat.current_keyframe]) ; else {
...
tyrano.plugin.kag.tag.kanim = {
    vital: ["keyframe"], pm: {name: "", layer: "", keyframe: ""}, start: function (pm) {
        var that = this, anim = this.kag.stat.map_keyframe[pm.keyframe], class_name = "." + pm.name;
        anim.config = pm;
...
tyrano.plugin.kag.tag.stop_kanim = {
    pm: {name: "", layer: ""}, start: function (pm) {
...
tyrano.plugin.kag.tag.chara_ptext = {
    pm: {name: "", face: ""}, start: function (pm) {
        this.kag.layer.hideEventLayer();
        if ("" == pm.name) {
            ...
            if (cpm) {
                ...
                if ("none" != this.kag.stat.chara_talk_anim) {
                    ...
                    if (chara_obj.get(0)) {
                        ...
                        "" != pm.face && this.kag.ftag.startTag("chara_mod", {name: pm.name, face: pm.face, time: "0"})
                    }
                }
            } else {
        ...
        if (1 == this.kag.stat.vostart && this.kag.stat.map_vo.vochara[pm.name]) {
            var vochara = this.kag.stat.map_vo.vochara[pm.name], se_pm = {
                loop: "false",
        ...
        if ("" != pm.face) {
            if (!this.kag.stat.charas[pm.name].map_face[pm.face]) {
                this.kag.error("Указанный персонаж「" + pm.name + "」или лицо:「" + pm.face + "」не определено. Пожалуйста, проверьте еще раз");
                return
            }
        ...
        if (void 0 === this.kag.tmp.map_chara_talk_top[name]) {
            var that = this, tmp_top = parseInt(chara_obj.get(0).offsetTop);
...
tyrano.plugin.kag.tag.chara_new = {
    ...
    start: function (pm) {
        ...
        this.kag.preload(storage_url, (img_obj => {
            if (img_obj) {
                let img_width = $(img_obj).get(0).width, img_height = $(img_obj).get(0).height;
        ...
        if (null != cpm) {
            ...
            if ("true" != cpm.is_show) {
                ...
                for (key in chara_layer) {
                    var chara_part = chara_layer[key], current_part_id = chara_part.current_part_id,
                        chara_obj = chara_part[current_part_id];
                    "allow_storage" == current_part_id && (chara_obj = {
                        storage: chara_part.allow_storage,
                        visible: "true"
                    });
                    var part_storage = "./data/fgimage/" + chara_obj.storage, j_img = $("<img />");
                ...
                this.kag.preloadAll(array_storage, (function () {
                    ...
                    "true" != pm.wait && that.kag.ftag.nextOrder();
                    j_chara_root.stop(!0, !0).animate({opacity: "show"}, {
...
tyrano.plugin.kag.tag.chara_hide = {
    ...
    start: function (pm) {
        var that = this, target_layer = this.kag.layer.getLayer(pm.layer, pm.page),
                ...
                duration: parseInt(that.kag.cutTimeWithSkip(pm.time)), easing: "linear", complete: function () {
                    ...
                    if ("true" == that.kag.stat.chara_pos_mode && "true" == pm.pos_mode) {
                        ...
                        $(array_tyrano_chara).each((function () {
                            chara_num++;
                            tmp_base += base;
                            var j_chara = $(this), center = Math.floor(parseInt(j_chara.css("width")) / 2),
...
tyrano.plugin.kag.tag.chara_hide_all = {
    ...
    start: function (pm) {
        var that = this, img_obj = this.kag.layer.getLayer(pm.layer, pm.page).find(".tyrano_chara");
        that.kag.layer.hideEventLayer();
        var flag_complete = !1, charas = this.kag.stat.charas;
        ...
        if (img_obj.get(0)) {
            ...
            if ("false" == this.kag.stat.chara_memory) for (key in this.kag.stat.charas) this.kag.stat.charas[key].storage = this.kag.stat.charas[key].map_face.default;
            if ("true" != pm.wait) {
                this.kag.layer.showEventLayer();
                this.kag.ftag.nextOrder()
            }
...
tyrano.plugin.kag.tag.chara_mod = {
    vital: ["name"],
    pm: {name: "", face: "", reflect: "", storage: "", time: "", cross: "true", wait: "true"},
    start: function (pm) {
        var that = this;
        that.kag.layer.hideEventLayer();
        var storage_url = "", folder = "./data/fgimage/";
        if ("" != pm.face) {
            if (!this.kag.stat.charas[pm.name].map_face[pm.face]) {
                this.kag.error("Указанный персонаж「" + pm.name + "」или лицо:「" + pm.face + "」не определено. Пожалуйста, проверьте еще раз");
                return
...
tyrano.plugin.kag.tag.chara_move = {
    vital: ["name"],
    pm: {name: "", time: "600", anim: "false", left: "", top: "", width: "", height: "", effect: "", wait: "true"},
    start: function (pm) {
        var that = this, target_obj = $(".layer_fore").find("." + pm.name + ".tyrano_chara"),
            target_img = $(".layer_fore").find("." + pm.name + ".tyrano_chara").find("img");
        if (target_obj.get(0)) {
            var anim_style = {}, img_anim_style = {};
...
tyrano.plugin.kag.tag.chara_layer = {
    ...
    start: function (pm) {
        ...
        if (null != cpm) {
            ...
            var chara_part = {}, init_part = !1;
            if (chara_layer[pm.part]) chara_part = chara_layer[pm.part]; else {
                init_part = !0;
                cpm._layer[pm.part] = {default_part_id: pm.id, current_part_id: pm.id, zindex: pm.zindex}
            }
            var chara_obj = {};
...
tyrano.plugin.kag.tag.chara_layer_mod = {
    ...
    start: function (pm) {
        var cpm = this.kag.stat.charas[pm.name];
        if (null != cpm) if (cpm._layer) {
            cpm._layer[pm.part] && (cpm._layer[pm.part].zindex = pm.zindex);
            this.kag.ftag.nextOrder()
        } else this.kag.error("Указанный персонаж「" + pm.name + "」Для [chara_layer] не заданы части различия"); else this.kag.error("Указанный персонаж「" + pm.name + "」не создан черз [chara_new]")
    }
};
tyrano.plugin.kag.tag.chara_part = {
    vital: ["name"], pm: {name: "", allow_storage: "false", time: "", wait: "true"}, start: function (pm) {
        var that = this, cpm = this.kag.stat.charas[pm.name];
        if (null != cpm) if (cpm._layer) {
            var chara_part = cpm._layer, map_part = {}, array_storage = [];
            for (key in pm) if (chara_part[key]) {
                var part_id = pm[key];
            ...
            this.kag.preloadAll(array_storage, (function () {
                if ("" != pm.time) {
                    ...
                    for (key in map_part) !function () {
                        cnt++;
                        var part = map_part[key], j_img = target_obj.find(".part." + key), j_new_img = j_img.clone();
                        j_new_img.css("opacity", 0);
                    ...
                    for (key in map_part) {
                        var part = map_part[key], j_img = target_obj.find(".part." + key);
...
tyrano.plugin.kag.tag.chara_part_reset = {
    vital: ["name"], pm: {name: "", part: ""}, start: function (pm) {
        var cpm = this.kag.stat.charas[pm.name];
        if (null != cpm) if (cpm._layer) {
            var chara_part = cpm._layer, new_pm = {name: pm.name};
            if ("" == pm.part) for (key in chara_part) new_pm[key] = chara_part[key].default_part_id; else for (var array_part = pm.part.split(","), i = 0; i < array_part.length; i++) {
...
tyrano.plugin.kag.tag.filter = {
    ...
    start: function (pm) {
        ...
        j_obj.css({"-webkit-filter": filter_str, "-ms-filter": filter_str, "-moz-filter": filter_str});
        j_obj.addClass("tyrano_filter_effect");
        this.kag.ftag.nextOrder()
    }
};
tyrano.plugin.kag.tag.free_filter = {
    vital: [], pm: {layer: "", page: "fore", name: ""}, start: function (pm) {
        var j_obj;
