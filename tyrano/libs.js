(function ($) {
    ...
    $.loadText = function (file_path, callback) {
        $.ajax({
            ...
            error: function () {
                alert("Файл не найден:" + file_path);
                callback("");
            },
        });
    };
    ...
    $.getStorageWeb = function (key) {
        try {
            ...
            //alert("Функция сохранения недоступна в этой среде. Это происходит, например, при локальном запуске");
            //$.confirmSaveClear();
    ...
    $.confirmSaveClear = function () {
        if (
            confirm(
                "Данные сохранения могут быть повреждены. Вы хотите инициализировать сохраненные данные?",
            )
        ) {
            alert("Инициализация");
            localStorage.clear();
    ...
    $.getStorageCompress = function (key) {
        try {
            ...
            console.log(e);
            alert(
                "Функция сохранения недоступна в этой среде. Это происходит, например, при локальном запуске",
            );
            $.confirmSaveClear();
        }
